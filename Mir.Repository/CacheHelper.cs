...
namespace Mir.Repository
{
    ...
    public class CacheHelper : IDisposable
    {
        ...
        /// 
        /// </summary>
        /// <param name="dbNum">数据库顺序,从0开始</param>
        public CacheHelper(int dbNum = 0) : this(dbNum, null) { }
        ...
        public CacheHelper(int dbNum, string readWriteHosts)
        {
            DbNum = (int)dbNum;
            _conn =
                string.IsNullOrWhiteSpace(readWriteHosts) ? RedisConnectionHelp.Instance : connect.GetConnectionMultiplexer(readWriteHosts);
        }
        ...
        /// <param name="key">Redis Key</param>
        /// <param name="value">保存的值</param>
        /// <param name="expiry">保存截至时间</param>
        /// <returns></returns>
        public bool Set(string key, string value, DateTime expiry = default(DateTime))
        {
            key = AddSysCustomKey(key);
            ...
            {
                return Do(db => db.StringSet(key, value, expiry - DateTime.Now));
            }
        }
        ...
        /// <param name="expiry"></param>
        /// <returns></returns>
        public bool Set<T>(string key, T obj, DateTime expiry = default(DateTime))
        {
            key = AddSysCustomKey(key);
            string json = ConvertJson(obj);
            if (expiry == default(DateTime))
        ...
        {
            var prefixKey = CustomKey ?? connect.SysCustomKey;
        ...
        {
            string result = value is string ? value.ToString() : value.ObjectToJson();
            return result;
        }

        private T ConvertObj<T>(RedisValue value)
        {
            //return value.ObjectToJson().ToObject<T>();  //此处原来是这个转换格式,在转换时 RedisValue.ObjectToJson() 出现错误,固改成 RedisValue.ToString()
    ...
    public class RedisConnectionHelp
    {
        ...
        public string SysCustomKey
        {
            get
            {
                return connectionSettings.CustomKey;
            }
        }

        //"127.0.0.1:6379,allowadmin=true
        //private string RedisConnectionString = connectionSettings.RedisConnection;
            ...
            get
            {
                if (_instance == null)
                {
                    lock (Locker)
                    {
                        ...
                        {
                            _instance = new RedisConnectionHelp().GetManager();
                        }
                    }
                }
                return _instance;
        ...
        {
            connectionString = connectionString ?? connectionSettings.RedisConnection;
        ...
        {
            LogFileHelper.WriteLog("Redis的配置文件被更改: " + e.EndPoint, Commons.Log.LogType.Info);
        }
        ...
        {
            LogFileHelper.WriteLog("Redis遇到错误: " + e.Message, Commons.Log.LogType.Error);
        }
        ...
        {
            LogFileHelper.WriteLog("Redis重新建立连接之前发生错误: " + e.EndPoint, Commons.Log.LogType.Error);
        }
        ...
        {
            LogFileHelper.WriteLog("Redis重新连接：末端连接失败-> " + e.EndPoint + ", " + e.FailureType + (e.Exception == null ? "" : (", " + e.Exception.Message)), Commons.Log.LogType.Error);
        }
        ...
        {
            LogFileHelper.WriteLog("Redis集群:新目标-> " + e.NewEndPoint + ", 就目标-> " + e.OldEndPoint, Commons.Log.LogType.Info);
        }
        ...
        {
            LogFileHelper.WriteLog("Redis内部错误:消息-> " + e.Exception.Message, Commons.Log.LogType.Error);
        }
